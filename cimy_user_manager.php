...
Copyright (c) 2007-2016 Marco Cimmino
...
add_filter('send_password_change_email', 'cimy_um_send_password_change_email', 10, 3);

function cimy_um_send_password_change_email($send, $user, $userdata) {
	return empty($_POST["cimy_um_import"]) ? true && !empty($send) : false;
}
...
?>
			<td>
				<input type="checkbox" name="db_excel_compatibility" value="1"<?php checked(true, isset($_POST['db_excel_compatibility']), true); ?> />  <?php _e("Select this option if you want more Microsoft Excel compatibility", $cimy_um_domain); ?>
			</td>
		</tr>
		<tr>
	...
	while (!feof($fh)) {
		...
		if (($wp_user->ID != $wp_userid) || ($wp_user->ID == 0)) {
			...
			if (is_wp_error($wp_userid)) {
				$results["error"][] = sprintf(__("%s, dropped this new user: '%s'", $cimy_um_domain), esc_attr($wp_userid->get_error_message()), esc_attr($username)).$line_tr;
				continue;
			}
			$wp_user = new WP_User($wp_userid);
			...
			if (!empty($_POST["db_send_email_new_users"])) {
				$email_passw = null;
				if (function_exists("cimy_get_options")) {
					$options = cimy_get_options();
					if (!empty($options["email_include_plaintext_password"])) {
						$email_passw = $passw;
					}
				}
				wp_new_user_notification($wp_userid, $email_passw, "both");
			}
